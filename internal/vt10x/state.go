...
// SetScrollCallback registers fn, called with the lines that just scrolled off
// the top of the screen, oldest first. It fires only for scrolls that a real
// terminal would push into its scrollback: the scroll region starts at row 0
// and the normal (non-alternate) screen is active. Line deletion and scrolls
// inside a region below row 0 do not fire it.
func (t *State) SetScrollCallback(fn func(lines [][]Glyph)) {
	t.scrollOut = fn
}

// scrollUpTop scrolls the region up by n from its top edge, reporting the lines
// that leave the screen to the scroll callback.
func (t *State) scrollUpTop(n int) {
	if t.scrollOut != nil && t.top == 0 && t.mode&ModeAltScreen == 0 {
		if k := clamp(n, 0, t.bottom-t.top+1); k > 0 {
			out := make([][]Glyph, k)
			for i := range out {
				out[i] = append([]Glyph(nil), t.lines[t.top+i]...)
			}
			t.scrollOut(out)
		}
	}
